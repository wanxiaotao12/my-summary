...
| 组成部分 | 说明 | 示例 |
|----------|------|------|
| 源路径 | 本地文件路径，可以是**相对路径**（相对于 Dockerfile 所在目录）或**绝对路径**，可以是**单个文件**、**整个目录**或**Dockerfile 所在目录** | `requirements.txt`（单个文件）、`src/`（整个目录，递归复制）、`.`（Dockerfile 所在目录下的所有文件） |
...
| 类型 | 示例 | 含义 |
|------|------|------|
| 单个文件 | `requirements.txt` | 只复制这一个文件 |
| 目录 | `src/` | 复制目录下的**所有内容**，包括子目录和子目录中的文件（递归复制） |
